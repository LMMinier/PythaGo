...
import type { GameAction } from '../src/lessons';
...
const ART_ROOT='https://raw.githubusercontent.com/LMMinier/PythaGo/main/public/game/';
...
export async function createGame(parent:HTMLElement,_base:string):Promise<GameController>{
  const P:any=await loadPhaser();
  class Scene extends P.Scene{
    hero!:Phaser.GameObjects.Sprite;
    byte!:Phaser.GameObjects.Sprite;
    terminals=new Map<string,Phaser.GameObjects.Container>();

    constructor(){super('borough');}

    preload(){
      this.load.image('city',ART_ROOT+'city-background.svg');
      this.load.image('hero',ART_ROOT+'hero.svg');
      this.load.image('byte',ART_ROOT+'byte.svg');
    }
    ...
    create(){
      ...
      this.terminal(690,345,'street_display','SIGNAL');
      this.terminal(790,365,'tunnel_gate','GATE');
      this.terminal(110,365,'architect_core','CORE');
      this.game.events.on('move',this.move,this);
      this.game.events.on('actions',this.handleActions,this);
      this.game.events.on('celebrate',this.celebrate,this);
    }
    ...
    terminal(x:number,y:number,id:string,label:string){
      ...
      const container=this.add.container(x,y,[glow,body,screen,text]).setDepth(7);
      this.terminals.set(id,container);
    ...
    move(direction:Direction){
      ...
      if(direction==='down')y+=25;
      x=P.Math.Clamp(x,65,835);
      y=P.Math.Clamp(y,245,410);
      if(direction==='left')this.hero.setFlipX(true);
      if(direction==='right')this.hero.setFlipX(false);
      this.tweens.add({targets:this.hero,x,y,angle:direction==='up'?-2:direction==='down'?2:0,duration:150,ease:'Sine.Out'});
      this.tweens.add({targets:this.byte,x:x+105,y:y+28,duration:380,ease:'Sine.Out'});
    }

    handleActions(list:GameAction[]){list.forEach((action,index)=>this.time.delayedCall(index*260,()=>this.action(action)));}

    action(action:GameAction){
      if(action.type==='say'){this.speech(action.speaker,action.text);return;}
      if(action.type==='activate'&&action.target.toLowerCase()==='byte'){this.flash(this.byte,0x63efff);return;}
      if(action.type==='recharge'){this.flash(this.byte,0xffcf52);return;}
      const terminal=this.terminals.get(action.target);
      if(terminal)this.flash(terminal,action.type==='unlock'?0xffcf52:0x64f58e);
    }
    ...
    speech(speaker:string,message:string){
      ...
      const text=this.add.text(215,103,message,{fontFamily:'Arial',fontSize:'17px',color:'#f4fbff',wordWrap:{width:470}}).setDepth(41);
  ...
  const game=new P.Game({
    ...
    antialias:true,
    scene:Scene,
    scale:{mode:P.Scale.FIT,autoCenter:P.Scale.CENTER_BOTH,width:900,height:500}
  });

  return {
    move:(direction)=>game.events.emit('move',direction),
    actions:(actions)=>game.events.emit('actions',actions),
    celebrate:()=>game.events.emit('celebrate'),
