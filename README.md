...
- **Build profile:** `production` for store builds, `preview` for an internal Android APK, or `development` for a development-client build
...
If EAS shows `Failed to read "/main/package.json"`, the build form is pointing at a nonexistent subdirectory. Edit the GitHub build settings and clear **Base directory** so it is empty, then rerun the build from Git ref `main`.
